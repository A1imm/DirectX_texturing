...
void RenderWidget::Initialize()
{
	...
	LoadDDSTexture(L"gg_logo_sama_kostka.dds");
	LoadTexture(L"WoodCrate3.jpg");
...
void RenderWidget::BuildRootSignature()
{
	...
	resourceTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0);

	CD3DX12_ROOT_PARAMETER slotRootParameter[2];
	slotRootParameter[0].InitAsConstantBufferView(0); // constant buffer
	slotRootParameter[1].InitAsDescriptorTable(1, &resourceTable, D3D12_SHADER_VISIBILITY_PIXEL); // tekstura

	const CD3DX12_STATIC_SAMPLER_DESC sampler(
		0, // shaderRegister
		//D3D12_FILTER_MIN_MAG_MIP_POINT, // filter
		//D3D12_FILTER_MIN_LINEAR_MAG_MIP_POINT, //bilinearne filtrowanie
		D3D12_FILTER_ANISOTROPIC, //anizotropowe filtrowanie
...
void RenderWidget::LoadTexture(const wchar_t* path)
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE hDescriptor(m_srvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
	UINT incrementSize = m_dxDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	hDescriptor.Offset(1, incrementSize);
	...
	srvDesc.Texture2D.MipLevels = m_textureResource->GetDesc().MipLevels;
